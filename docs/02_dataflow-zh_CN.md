...
- $\mathrm{PE}(i,j)$ 负责 $C[i][j]$；
- $A[i][k]$ 沿第 $i$ 行从左向右传播；
- $B[k][j]$ 沿第 $j$ 列从上向下传播；
- $\mathrm{PE}(i,j)$ 在相同 $k$ 的 A、B 同拍到达时执行 MAC；
- partial sum 始终保存在 $\mathrm{PE}(i,j)$ 内部，直到 operation 完成。
...
因此，相同 $k$ 的 $A[i][k]$ 与 $B[k][j]$ 会在 $\mathrm{PE}(i,j)$ 同拍相遇。
...
$\mathrm{PE}(i,j)$ 执行第 $k$ 项 MAC 的周期为：
...
- 首次有效 MAC： $i+j$；
- 最后一次有效 MAC： $i+j+K-1$；
- 最早开始的是 $\mathrm{PE}(0,0)$；
- 最晚完成的是 $\mathrm{PE}(N-1,N-1)$。
...
6. $\mathrm{PE}(i,j)$ 只累加属于 $C[i][j]$ 的乘积；
