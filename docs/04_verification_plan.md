...
- 当前证据边界：已在三种参数配置、300 轮可复现随机 operation 中证明结果、完成周期和每 PE MAC 次数；协议 assertions、完整参数边界与定向 corner-case 扩展仍待完成
...
三种配置均使用 `DATA_W=8`、`ACC_W=18`，并在 Verilator `--Wall` 下无 warning。共完成 300 轮随机 operation。
...
4. 每个 PE 每轮恰好执行 $K$ 次 MAC：三种随机配置、300 轮 operation 已由层次化 monitor 直接证明；
...
当前尚未满足的主要退出项为：随机参数边界 $N=1,K=1$、$N=2,K=1$、$N=4,K=1$，定向 corner cases，以及第 7 章所列协议性质的系统化 assertion/monitor。第一阶段随机回归和每 PE MAC 次数检查已经关闭。
