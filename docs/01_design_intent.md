...
- 状态：第二版，baseline 架构已冻结，等待首次编译与仿真
- 当前覆盖范围：MVP 目标、系统边界、核心设计原则、外部操作协议与验收标准
- 已实现但尚未验证：PE、裸阵列、Input Feeder、Controller、系统顶层及对应定向 testbench
...
截至当前 checkpoint，五组 testbench 已经编写完成，但尚未实际编译或仿真。因此上述验收项均不能仅凭 RTL 和 testbench 文件存在而标记为通过。
...
当前最优先事项不是继续扩展 RTL，而是依次编译并运行：

1. `tb_systolic_pe.sv`；
2. `tb_systolic_array.sv`；
3. `tb_input_feeder.sv`；
4. `tb_systolic_controller.sv`；
5. `tb_systolic_array_top.sv`。

首次回归通过后，仍需决定或验证：

1. 参数化随机回归和每个 PE 恰好执行 $K$ 次 MAC 的检查方式；
2. `N=4` 配置的功能、利用率和 PPA；
3. 更大 $K$ 下 `ACC_W` 的推导和溢出策略；
4. 未来是否增加矩阵装载存储、索引读取结果或流式接口；
5. baseline 索引式 feeder 与延迟链式 feeder 的结构和 PPA 差异。
